\begin{abstract}
Existing event extraction systems are typically constructed using supervised
learning. The effectiveness of these systems heavily rely on the quality of
expert-annotated datasets. However, the current approach of corpora construction
remains a manual, time-consuming process.  As a result, the generated datasets
often only cover a limited class of event types, making the
learned extractor hard to generalize. In this paper, we address the problem
of how to automatic construct training datasets of rich event types. We achieve this by
employing distant supervision to automatically extract events from
unlabelled data using structured knowledge bases. We propose a novel
neural network with ILP-based post inference, committing to handling two
challenges in event extraction: multi-type events and multi-word arguments.
\FIXME{We evaluate our approach by applying it to ...}. Both automatic and
manual evaluations demonstrate that it is possible to learn to effectively extract
various  events without
