\begin{abstract}
Existing event extraction systems are typically investigated in supervised
learning paradigm. The effectiveness of these systems heavily relies on the quality of
expert-annotated datasets, which still require a costly and time-consuming construction
process.  As a result, the built datasets
often only cover a limited variaty of event types, making the
learned event extractor hard to generalize. In this paper, we address the problem
of automatically building event extractors for rich event types without much expert involvment. 
...
the corresponding event arguments involved.
We evaluate our approach by automatically collecting trainding data for many event types from Freebase, where 
